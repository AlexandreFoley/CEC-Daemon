    ...
    for name, func in inspect.getmembers(cls):
        if hasattr(func, '__cmds__'):
            for cmd in func.__cmds__ :
                cls.help_string = cls.help_string + cmd + ' '
                cls.interactive_cmd[cmd] = func
            cls.help_string = cls.help_string + '\t' + func.__doc__ + '\n'
    return cls
...
class pyCecClient:
    ...
    @register_mainloop_command('self')
    ...
    def MainLoop(self):
        if self.adapter:
            ...
            print("initialize the CEC client first!")
